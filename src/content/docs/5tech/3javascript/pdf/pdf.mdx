...
description: pdfMakeとarchiverを使って複数PDFをZIP化しStorageへ保存する手順を解説。Cloud Functionsでの生成処理と配布フローの実装例を紹介します。
sidebar:
...
## 1つのPDFファイルをNode.jsで作成する [#id=create_single_pdf_with_nodejs]
...
## PDFファイルを生成せずに直接アーカイブする（複数ファイル） [#id=archive_multiple_files_without_generating_pdf]
...
## 非同期処理とCloud Functionsのライフサイクルに注意する [#id=functions_lifecycle]
