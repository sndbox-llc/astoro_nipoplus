...
description: async/await利用時にエラーが期待どおり伝播しない原因と対処を解説。try-catchの置き方やPromiseの扱い方を、実例ベースでわかりやすく整理しています。
sidebar:
  label: async関数の落とし穴
---
...
## async/awaitの見落としがちな落とし穴。try/catchしても呼び出し元でcatch出来ません [#id=tryAndCatch]
...
## なぜfunc1はエラー発生時に止まらないのか？ try・catchを利用しても止まらない理由 [#id=asyncAwaitTrap]
...
## そもそもawaitの結果はresolveかrejectと考えれば止まらないのも道理 [#id=asyncAwaitResolveReject]
...
## throw したあとでもfinallyは実行されます [#id=finally_executes_even_after_throw]
