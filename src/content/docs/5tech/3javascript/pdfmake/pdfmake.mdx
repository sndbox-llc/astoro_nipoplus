...
description: pdfMakeのページサイズ指定で迷いやすい単位系を解説。mm換算の考え方やサイズ調整の実例を通じて、期待どおりのPDFレイアウトを作る方法を紹介します。
sidebar:
...
## pdfMakeの単位はmm（ミリ）じゃないよ [#id=pdfmake_units_are_not_mm]
...
### SPAでPDFが作れるのは便利だが注意点も [#id=warnings]
