...
## Register Service Worker [#id=register_service_worker]
...
## Quasar公式サイトの真似をしてみる [#id=imitate_quasar_official_website]
...
## iPadやAndroidで更新時にアプリがフリーズするトラブルが発覚 [#id=trouble_with_app_freezing]
...
## Service worker上ではなくVue上で更新処理を行うように修正 [#id=update_process]
...
### register-service-worker.jsで更新イベントを発行する [#id=firedUpdateEvent]
...
register(process.env.SERVICE_WORKER_FILE, {
  ...
  updated(registration) {
    // ▼ここを修正しています‼️イベント名は参考にさせていただいたサイト様に敬意を評し、同じイベント名を使わせていただきました
...
### App.vue側でswUpdateイベントを広い、更新通知を行う [#id=noticeFrontEnd]
...
### WebpackのOptionsでskipwaitingを無効化しておくことを忘れずに [#id=skipWaiting]
