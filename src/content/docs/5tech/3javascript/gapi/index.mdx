...
## gapiよ、なぜ君はnode moduleに登録されていないのか？ [#id=gapi]
...
## VueJSでGapiを使えるようにしよう [#id=vue_use_gapi]
...
## TypeScriptにGapiを認識させるまでの流れ [#id=how_to_use_gapi]
...
の3つです。これらをそれぞれインストールします。このインストール自体はおなじみの npm install でできるので簡単です。
...
## gapi is not definedを回避するためのdeclare [#id=fixed_declare]
...
## front endで実装するべきか、サーバ側で実装するべきか？ [#id=question]
