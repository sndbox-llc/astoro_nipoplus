...
description: onAuthStateChangedの正しい使い方を、重複発火や初期化順序の落とし穴とあわせて解説。Vue環境で安定した認証状態監視を実現する実装指針を示します。
sidebar:
...
## Vue.js3でonAuthStateChangedを登録する [#id=vue3resist]
...
このloginStart関数が2度呼び出されてしまいます。何度呼ばれても大丈夫なように冪等性を考慮して開発していれば大きな問題にはなりませんが、それでもあまり気持ちのいいものではありません。
...
## コンポーネント破壊時にonAuthStateChangedをunsubscribeする [#id=unsubscribe_component_destroy]
...
### ログアウトの検知もできなくなるので注意 [#id=logoutWarning]
