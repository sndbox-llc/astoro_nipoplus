...
## FireStoreのクエリは貧弱である [#id=fireStoreQuery]
...
## Elastic Searchとは？ [#id=aboutElasticSearch]

ElasticSearchは全文検索に対応したデータベースです。FireStoreではできない全文検索や高度なクエリを行うことができます。オープンソースで開発されており、自前でサーバを用意する場合は無料で利用できます。
自前でサーバを用意できない場合はElastic Cloudというサーバを有償で利用できます。NipoPlusはElastic Cloudを使用しています。

## FirestoreとElasitc Searchを組み合わせて使う [#id=convinationFsAndEs]
...
### Elastic Searchの準備 [#id=setupElasticSearch]
...
async function makeElastic() {
  ...
  try {
    await esClient.indices.create(先ほど作ったスキーマJSON)
    console.info('○○○○ 成功 ○○○○')
  } catch (e) {
...
### Cloud FunctionsでFirestoreのデータをElastic Searchへプッシュする [#id=documentMerge]
...
export const esPushReport = functions.firestore.document('group/{groupId}/document/{documentId}').onWrite(async (change, context) => {
  ...
  const index = '先ほど作ったElasticのインデックス名';
...
## データの検索はCloud Functionsを経由して行う [#id=usageCloudFunctions]
...
## 他の全文検索データベースとの比較 [#id=otherDatabaseInfo]
