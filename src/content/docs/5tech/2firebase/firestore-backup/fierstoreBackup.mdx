...
description: Firestoreバックアップと復元の実践手順を紹介。公式機能の設定方法、部分復元の検証、運用時に押さえるべき注意点を具体例で解説します。
sidebar:
...
## ポイントインタイム リカバリによりFirebaseのデータをバックアップできるようになります [#id=firestore_backup]

いやぁ、ついに来ましたね。2023年夏、Firestoreはこれまでまともなバックアップ機能がありませんでしたが、ついに公式からバックアップ機能が実装されました。

詳しくは[本家記事](https://firebase.google.com/docs/firestore/pitr?hl=ja)をご覧ください。
...
インクリメントの実装や配列内の検索機能、Collection Groupなんかも以前はありませんでした。
そして今回ついにバックアップの実装。いやはや、Firebaseはどんどん便利になっていきますね。

## ポイントインタイムリカバリを使って実際にデータを一部のみ復元してみた [#id=use_recovery]
...
2. 復元用のFirestoreを作る
3. 復元用のFirestoreにデータを流し込む
4. 復元用のFirestoreから本稼働のFiresoreに復元したいドキュメントのデータを流し込む
...
### 一時保管用バケットを作る [#id=make_temp_bucket]

復元したいとき点のFirestoreスナップショットをBucketにコピーします。
GCPにログインし、適当なBucketを作っておきましょう。

![GCPにログインして一時保管用のバケットを作っておきます。バケット名はあとから使うので名前を控えておきます](./img/createBuket.png)

ここではバケット名を nipoplus_tmp とします。バケット名はあとから使うので忘れずにメモしてください。

バケット名: nipoplus_tmp

### 復元したいとき点でのスナップショットを先ほど作ったバケットに保存します [#id=upload_snapshot]
...
### 復元用のFirestoreを作り、スナップショットのデータを復元用Firestoreにインポートする [#id=import_firestore]
...
![Firestoreに切り替えて一覧に先ほど作ったFirestoreが追加されていることを確認してください。反映されない場合はリロードします](./img/firestore_list.png)
...
### 復元DBから本番DBへ一部のデータを移動する [#id=move_test_to_product]
 ...
 * @param dist 書き込み先のパス。srcと同じパスを指定すれば、復旧用DBー＞本番用DBへ同じパス上に書き込みされます。異なるパスを指定できます
 */
...
緊張の一瞬ですね。こういう本番のデータベースをいじるのは心臓に悪いのでできればやりたくありません。
...
Firestoreは非常に優れた製品です。しかしバックアップと復元はまだちょっと不便なところが少しあると感じました。
