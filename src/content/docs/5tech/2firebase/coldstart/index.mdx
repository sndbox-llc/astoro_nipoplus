...
## Cloud SchedulerとPub/subを使って定期的にCloud Functionsを実行する [#id=usePubSub]
...
JWTを取得してアクセスする方法もありますが、フロントのログインと同じように書ける方法が好みです。
node.js上でFirebaseにログインし、Cloud Functionsに定期的にアクセスするような処理にしてみました。
以下、コードです。
...
Firebaseにログインする必要があるため、この定期予定実行するためのアカウントをFirebase上に登録しておく必要があります。
...
## Cloud schedulerに登録されていることを確認する [#id=registrationCloudScheduler]
...
## 定期実行される関数は特定のパラメータがきたらすぐ抜けるように修正 [#id=modify_periodic_function_to_exit_immediately]

定期的にCloud Functionsを実行するわけですが、バカ正直に最後まで走らせる必要はありません。まぁ普通に考えればパラメータ不足などで途中でエラー終了になるパターンが多そうですが。
呼び出される側の関数で、特定のパラメータが来たらすぐに抜けるような処理を書いてあげます。
...
## 実際に実装してみて [#id=try_implementing_in_real]
...
6分のサイクルが適切かは正直定かではありませんが、今の所体感としてほぼ問題なさそうです。
