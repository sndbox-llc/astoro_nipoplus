...
sidebar:
  label: Cloud Functionsのコールドスタートを回避する
---

:::note[Cloud Functions2を使おう]
...
この1サイクルで複数のCloud Functionsを実行する場合はその数だけ倍々に増えていきます。
...
![GCPの管理画面からScherulerを表示した](./img/scheduler.png)
...
export default functions.https.onCall(async (data, context) => {
  ...
  if ('wakeup' in data) {
    ...
    functions.logger.log(`朝だぞ‼️起きろ ${codeHead}`)
