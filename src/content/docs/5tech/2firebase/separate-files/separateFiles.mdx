...
## FirebaseにおけるCloud Functions [#id=about_cloud_functions]
...
## クラウドファンクションのファイルを関数単位で分けよう [#id=files]
...
先ほどのサンプルコードを例に、addDataCounterを別のファイル（myCounter.ts）に切り分けてみましょう。
