...
## AlgoliaからTypeSenseへ切り替えは可能か？ [#id=switching_from_algolia_to_typesense]
...
使い方も簡単で、速度も申し分ないのですが料金がネックです。
この度、Nipoではない他のプロダクトで試験的にTypeSenseを導入してみました。このページはTypesense導入におけるお話をしていきます。実際に使ってみて感じたことをつらつらと書いていきます。

## TypeSenseは低価格で全文検索が可能な新しいデータベースです [#id=typesenseCanFulltextSearch]

Typesenseの情報自体はまだまだ少ないです。日本語によるTypesense導入を解説しているページはほとんどありません。Typesenseがどのようなものかというと、メモリ上だけで保存される全文検索が可能なデータベースです。
...
### リレーショナルデータベースのようにテーブル構造を作成する必要があります [#id=needsSchemaSetting]
...
## 半角スペースで単語を区切らない日本語は全文検索に工夫が必要です [#id=full_text_search_for_cjk]
...
Typesenseへの書き込みなんかは、公式サイトの[データ書き込み](https://typesense.org/docs/0.21.0/api/documents.html#index-a-document)をご覧ください。
...
## 他の会社のデータを見れないように制限付きAPIキーをうまく使おう [#id=securityManagement]
...
制限付きキーの作り方について詳しくは[Typesense公式ガイド-API Key](https://typesense.org/docs/0.21.0/api/api-keys.html#generate-scoped-search-key)をご覧ください。
...
## TypeSenseのイマイチなポイント [#id=weak_points_of_typesense]

非常に便利なTypesenseですが、イマイチなポイントもありました。いくつかまとめてみます

### 「文字列」のソートには対応していない [#id=does_not_support_sorting_strings]
...
### 日本語など半角スペースで区切らない言語に厳しい [#id=fulltext_CJK]
...
### インメモリで動くためサーバ構成を変えることができない [#id=staticServerSpec]
...
### ライセンスがGPL3である（用途によっては問題になるかも？） [#id=licensed_under_gpl3]
...
### TypeScriptに対応していない [#id=does_not_support_typescript]
...
## 検索ノイズが多いのでもう少し改良した話 [#id=furtherNoise]
