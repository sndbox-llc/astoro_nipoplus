...
description: CORSエラーによってCloud Functionがうまく動かなかった。その原因と解決策に奔走してしまい、3時間を無駄にしました。原因と解決策についてまとめています
sidebar:
  label: CORS問題を回避
---
...
といった点がありますが、実は無視できない大きなデメリットが1つあります。それが 「問い合わせフォームでメール送信ができない」という問題です。
...
export default functions.https.onRequest(async (req, res) => {
  res.set('Access-Control-Allow-Origin', '*') // 追加した1行。これでどのサイトからもリクエストできるようになる？
...
解決までに3時間も費やしてしまいました。参考にさせていただいたサイトたち
