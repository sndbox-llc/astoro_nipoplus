...
![プロジェクトを有効にする](./img/enableProject_tablet.webp)
...
![プロジェクトの工程を作る](./img/projectSetting_tablet.webp)
![サンプルで２つの工程を作ってみました](./img/projectSetting2_tablet.webp)
...
1. メニューから「プロジェクト」を選択
2. 「プロジェクトを作る」のボタンをクリック
3. プロジェクト名を入力
4. 保存ボタンをクリック

![プロジェクト本体を作ってみよう](./img/makeProject_tablet.webp)

これでプロジェクトが作成されました。以下はサンプルで３件ほどのプロジェクトを追加した画面イメージです。

![プロジェクトを３件ほど登録した画面イメージ](./img/projectList_tablet.webp)
...
![プロジェクト画面から日報作成するとプロジェクト情報が自動でセットされる](./img/makeReport_tablet.webp)

内容を書き上げて、提出ボタンを押すと、当該日報が「プロジェクト」として保存されます。
また、通常の受信一覧にも登録されます。

![日報が提出されるとボタンが日報へのリンクに変わる](./img/projectWrited_tablet.webp)
...
状態は「進行・完了・破棄」の３つです。必要に応じて変更できます。

![完了や破棄など状態を変更](./img/projectStatus_tablet.webp)
