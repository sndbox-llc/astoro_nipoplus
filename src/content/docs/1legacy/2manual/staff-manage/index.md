...
グループにどのスタッフが所属しているかを確認するには、全体設定＞メンバー管理の順にクリックします。

![ルームに所属しているスタッフ一覧を表示](./img/staffList_tablet.png)
...
スタッフを追加する際には、E-mailアドレスが必要になります。
スタッフを追加するのはかんたんです。E-mailアドレスを入力し、スタッフ追加ボタンをクリックするだけです。

![mailを入力してスタッフを追加する](./img/staffMake_tablet.png)
...
:::caution[初期パスワードは安全とは言えませんのでログイン後にパスワードの変更を推奨します]
:::
...
つまり初期パスワードは無視され、これまでどおりのパスワードでログインするすることになります
...
ちょうど話に出てきたので、「管理者」「マネージャ」などのスタッフ権限について見てみましょう
...
![スタッフの権限を変更する](./img/changeStaffRank_tablet.webp)

#### 管理者権限について {#admin}

グループの管理ができる人です。管理者は1グループに1名までです。最初にグループを作った人が管理者になりますが、必要に応じて管理者の権限を他のスタッフに譲ることもできます。
...
Nipo【旧版】では日報を作成する際に、誰あてに日報を提出するのかを日報作成者自身で選ぶことができます。
それとは別に管理者側が各スタッフに対して日報の提出先を強制的に指定することもできます。

:::note[例えば「Aさんの書く」「業務日報は」「Cさん宛てで固定する」という具合です]
:::
...
![テンプレート毎に提出先や共有先の設定を行える](./img/reportDistLock_tablet.webp)
...
![スタッフの活動実績を表示する](./img/staffActivity_tablet.png)
...
活動実績は無料PLANの場合、「当月分」のみ読むことができます。月の切替には[GOLDPLAN](/legacy/system/price/)への加入が必要です。
