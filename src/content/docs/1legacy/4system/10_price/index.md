...
![請求書払いは完全先払い](./img/invoice_tablet.webp)

:::note[残存期間中の契約人数変更について]
...
![オンライン請求書](./img/payment_method_tablet.webp)

請求書の支払いはクレジットカードか、口座振込が可能です。クレジットカードで支払う場合は即座に決済が完了します。
口座振込の場合は最長で２営業日ほど時間が掛かります。決済が完了するまでGOLDPLANの延長手続きはされないため、ダウンタイムが発生しないように前もって余裕を持って決済するようにしてください。

### 請求書払いの更新について {#invoice_renewal}

更新は新規お申し込みと同じ手順で、請求書を発行し、支払いをしていただくことで行うことができます。
契約期間内での更新でも、現在の満期から計算して期間延長が行われますので少し前もって余裕を持った更新をお願いします。（無料PLANに降格するとダウンタイムが発生し、業務に支障をきたす可能性があります）
...
4. 新しい人数を入力する（変更後の人数を入力。例えば今10名で5名増やす場合、増減後の 15と入力）

![人数変更は月3回まで。今まで不可能だった人数減少も可](./img/edit-member-limit_tablet.webp)

## 自動更新有り（クレジットカード払い） {#newCredit}

:::danger[新規でのお申し込みは終了しました]
:::
...
![サブスクリプションの解約や変更はここから操作](./img/subscription-edit_tablet.webp)
...
![管理画面から領収証のダウンロード](./img/receipt1_tablet.webp)

<div class="next-arrow">▼</div>

![オンライン領収証からダウンロード](./img/receipt2_tablet.webp)
...
最も多い勘違いがAndroid・iOS版のNipo【旧版】で解約しようとすることです。
アプリストアの規約上、App Store・Google Play上に展開しているアプリから契約及び解約はできないようになっています。
...
![「課金を停止」ボタンを押下](./img/stop-subscription_tablet.webp)
