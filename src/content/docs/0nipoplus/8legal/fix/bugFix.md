...
NipoPlusをご利用中に、もし「あれ？なんだかおかしいな」と感じたり、画面が正常に表示されないなどの問題が発生した場合でも多くの場合、シンプルな操作で解決できます。
このページでは、よくあるトラブルとその解決策を、ステップバイステップでご紹介します。上から順番にお試しください。
...
:::caution[シャープマーク（#）より後ろはブックマークに入れないこと]
:::
...
予め[匿名アカウントを正規アカウントに昇格](/nipoplus/reference/tokumei/#enhance)の上操作してください。
:::
...
![検証パネルを表示する（Google Chormeのケース）](./img/kensho_tablet.webp)

画面下部に検証パネルが表示されました。アプリケーション＞ストレージ＞サイトデータを消去の順にクリックします。

![検証パネル画面](./img/console.webp)

このパネルで消去されるのはNipoPlusの一時データだけで、日報やコメントなど重要なデータは削除されません。
...
これまでの手順をすべて試しても問題が解決しない場合は、お手数ですが、以下の情報と共に[お問い合わせ](/inquery)ください。
