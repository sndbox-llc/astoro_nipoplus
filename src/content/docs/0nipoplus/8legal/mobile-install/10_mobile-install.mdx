...
NipoPlusのダウンロード用QRコードも用意しています。印刷の上ご活用ください。
...
<dl class="basic">
  ...
  <dd>
    ...
    /{' '}
    <a href="https://www.apple.com/jp/safari/" target="_blank">
      Safari
    </a>
  </dd>
  <dt>その他のブラウザ</dt>
  <dd>
    ...
    /{' '}
    <a href="https://www.opera.com/ja" target="_blank">
      Opera
    </a>
  </dd>
  <dt>その他</dt>
...
## Androidで使う場合 [#id=android_usage]

Google Play Storeからインストールするか、PWAとしてインストールする方法があります。

### PWAインストール【推奨】 [#id=android_pwa_install]
...
### Google Playストアからインストール [#id=android_usage]
...
## iOSで使う場合 [#id=ios_usage]

App Storeからインストールするか、PWAとしてインストールする方法があります。

### PWAインストール【推奨】 [#id=ios_pwa_add_home]
...
### App Storeからインストール [#id=ios_appstore_install]
...
<dl class="basic">
  ...
  <dt>推奨モニターサイズ</dt>
  <dd>
    iPhone:5.4インチ以上（iPhone SE2のサイズ）
    <br />
    iPad:9.7インチ以上
    <br />
    （これ以下のサイズでは操作性が低下する場合があります）
  </dd>
</dl>

---

## ストア版専用アプリの制限事項 [#id=store_app_limitations]
