...
title: 日報データをCSV出力する際の設定項目と列の展開仕様
...
<TopicGrid>
  <AutoTopicCard title="日報のCSV出力" href="/nipoplus/staff/csv" />
</TopicGrid>
...
<dl class="basic">
<dt>列の展開</dt>
<dd>1つのデータに対して複数列を使用して出力します。詳細は下記の「<a href="#splead">列の展開について</a>」を参照してください</dd>
<dt>行列を入れかえる</dt>
<dd>
  <ul>
...
<dt>写真・ファイル等を出力</dt>
<dd>
  <a href="/nipoplus/template/binarys/#picture">写真入力フォーム</a>、<a href="/nipoplus/template/binarys/#sign">署名入力フォーム</a>、
  <a href="/nipoplus/template/binarys/#file">ファイル入力フォーム</a>で添付されたデータをCSVファイルとは別のフォルダにまとめて出力します。また、ダウンロード形式がZIPに変化します
</dd>
...
<dd>
  ...
  <a href="/nipoplus/template/array">反復入力</a>がないテンプレートではこの設定は意味がありません
</dd>
...
列はCSVにおける縦方向のデータです。1つのデータに対して1列で表現するか、複数列で表現するかを選択できます。
文字だけでは伝わりづらいため、同一の日報データを列展開ON/OFFで出力した例を以下に記載します。
...
テンプレートでの設定が優先して適用され、その後にCSV出力設定が適用されます。
