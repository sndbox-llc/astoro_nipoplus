...
NipoPlusは、[様々な入力フォーム](/nipoplus/template/make/)を組み合わせて自由にカスタマイズされたテンプレート（日報、点検シート、各種業務報告書など）に沿って情報を入力します。10種類以上の入力フォームがあるため、フォームによっては入力方法が分かりにくいと感じる方もいらっしゃるかもしれません。
...
![シンプルな文字入力フォームです。日報の本文や、点検時の所見など、改行も使って自由に記述できます。URLを貼れば自動でリンクになります](./img/text-preview_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/text/)

シンプルな文字入力です。見た目は1行しか入力できそうにありませんが、改行を入れることで入力エリアが自動で広がります。これにより、長文の報告書や詳細な作業記録も問題なく入力できます。
...
![あらかじめ用意したリストから1つだけ選ぶ簡単入力です。日報の業務区分や、点検結果のステータス（例：正常/異常あり）などに](./img/select_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/selects/#plain)

該当する選択肢をタップやクリックで選択します。1つだけ選択可能です。
「入力モード切替」を押すとプルダウン・ラジオボタンを交互に切り替えます。
「入力モード切替」ボタンを非表示にしたい場合は、テンプレート[設定](/nipoplus/template/selects/#plain)から「入力切替を無効にする」をONにします。

### 選択入力（複数選択） {#select2}

![あらかじめ用意したリストから複数選ぶことができます。見た目はチェックボックスです。使用機材の選択や、該当する不具合箇所の複数記録などに](./img/select2_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/selects/#multiple)
...
![問1の選択によって問2の回答リストが変化します。例えば、エリアを選ぶと支店リストが変化。問2を忘れずに入力しましょう](./img/select2-preview_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/selects/#layerd)

日報作成直後の画面では問1しか表示されませんが、問1を選択すると自動で問2が出現します。問2の内容は問1の選択によって変化します。
入力必須が設定されている場合は、問2まで回答して初めて提出が可能になります。
この機能は現在非推奨であり、より柔軟な[動的変化機能](/nipoplus/template/selects/#dinamic_switch_select)のご利用を推奨します。

### チェックボックス {#checkbox}

![タップでON、もう一度タップでOFFと、直感的な2択入力です。タスクの完了チェックや、点検項目の合否判定に最適です](./img/checkbox-preview_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/selects/#checkbox)
...
![日報の作成時刻、点検実施日時、トラブル発生時刻など、特定の時点を記録できます。カレンダーや時計マークから簡単入力](./img/date-time_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/date_time/#point)

正しい形式で日付や時刻を入力します。カレンダーなどのデイトピッカーを使うことで正しい値を間違えずに入力可能です。直接キーボードから打ち込む際は入力ミスに注意してください。

### 日付と時刻（期間） {#datetimes}

![作業開始日時と終了日時を入力します。正しい日時が入力されると、その間の「経過時間」が自動で計算され表示されます。工数管理や設備稼働時間の記録に](./img/datetimes-preview_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/date_time/#range)
...
[▶このフォームを追加する方法](/nipoplus/template/binarys/#sign)

描画キャンバスは署名用とイラスト用の2種類があります。
署名用キャンパス▼

![スタイラスペンや指を使って、手書きのサインや簡単な図をキャンバスに描画できます。タブレットとの組み合わせが便利です](./img/write-signature_tablet.png)

イラスト用キャンバス▼

![署名と違い、より大きなキャンバスでイラストを描画できます。色変えなどの機能も使え、現場の状況を詳細に図示するのに役立ちます](./img/draw-stylus-pen_tablet.png)
...
製造現場の状況記録、設備点検時の異常箇所の撮影、作業完了の証拠写真などに最適です。

※[無料プラン](/nipoplus/price/free/)では低画質でアップロードされます。

![PCはドラッグ＆ドロップで写真を追加、スマホやタブレットの場合はボタンをタップしてカメラ起動や写真選択。現場の状況をその場で手軽に記録しましょう](./img/picinput_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/binarys/#picture)
...
![主にPCでの利用を想定しています。ドラッグ＆ドロップ、またはクリックで各種ファイルを日報に添付できます。設計図や検査成績書、関連資料の共有に](./img/file-input_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/binarys/#file)
...
半角のみ扱えます。マイナス、小数点も入力可能です。売上高、生産数、走行距離、訪問件数、測定値など、最も一般的な数値記録に利用します。

![キーボードから数値を入力します。タッチパネル対応デバイスなら専用テンキーが使いやすく、生産数や測定値などの入力もスムーズです](./img/math-input_tablet.webp)

[▶このフォームを追加する方法](/nipoplus/template/digital/#commonNumber)

### レート入力 {#rate}

クリック操作で任意の星の数を入力できる評価フォームです。同じ数をもう一度クリックすると未入力状態に戻ります。製品やサービスの満足度、作業の品質評価、検査項目の重要度などを直感的に記録するのに最適です。

![タップで星の数を選びます。同じ数をもう一度タップすると★0の状態に戻ります。直感的な評価入力に便利です](./img/rate-preview_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/digital/#rate)

### スライダ入力 {#slider}

ドラッグやスワイプでレール上のつまみを操作し、数値を入力します。キーボードを使わずに数値入力が可能です。レールの上限、下限を超えて入力することはできません。
気温、湿度、圧力、騒音レベルなど、特定の範囲内の数値を直感的に入力するのに適しています。

![ドラッグやスワイプでつまみを動かして数値を入力します。特定の範囲内の測定値や環境データを手軽に記録しましょう](./img/slider-preview_tablet.png)

[▶このフォームを追加する方法](/nipoplus/template/digital/#slider)
...
[▶このフォームを追加する方法](/nipoplus/template/digital/#range)
...
[▶このフォームを追加する方法](/nipoplus/template/digital/#calc)
...
[▶このフォームを追加する方法](/nipoplus/template/mod/)
...
[▶このフォームを追加する方法](/nipoplus/template/array/)
