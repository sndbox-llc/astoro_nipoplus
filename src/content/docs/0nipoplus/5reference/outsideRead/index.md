...
従来、アクセス権がない相手に日報を送る方法として[PDF出力](/docs/staff/readreport/#pdf_export)がありましたが、PDFに変換した後でメールに添付して送信するという一連の流れは、特に多くの日報を共有する際に手間に感じるかもしれません。
...
1.  共有したい日報を[保存箱](/docs/reference/searchreport/#listbox)などから表示する
...
リンクの作成には[スタッフ権限](/docs/reference/userRank/#staff)以上が必要です（書き込みのみ権限では生成できません）。
...
- **日報の削除による無効化:**
  - 共有リンクが発行された元の日報自体を[削除](/docs/reference/removereport/)した場合も、そのリンクは自動的に無効になります。
