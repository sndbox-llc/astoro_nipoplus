...
### キーワードにスタッフ名（名前・ふりがな・メモ）がマッチするケース {#fulltext_staff_search}

キーワードにスタッフ名（名前・ふりがな・スタッフメモ）がマッチする場合、検索が「スタッフモード」となります。
例えば「上田」というキーワードで検索した場合を例に考えてみます。スタッフに「上田」が存在する場合、２つの解釈ができます。

- 「日報本文に上田が含まれている」
- 「作成者が上田である」

この検索は同時には行えず、上記いづれかどちらかで検索がされます。スタッフ名にマッチした場合に限り、モードの切り替えスイッチが出現します。
必要に応じてこの切り替えスイッチを使って適切な検索を行ってください。

![スタッフによる検索](./img/stafffilter.webp)

本来、スタッフの検索は「スタッフ一覧」から検索することを想定しています。スタッフによる検索を見落としてしまう方のためにおせっかい機能の１つとして実装されております。
そのため、すでにスタッフによる絞り込みを手動で行っている場合、この機能は発動しません。
