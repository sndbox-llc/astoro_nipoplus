...
title: NipoPlusの外部連携APIの使い方 日報・タイムカードデータの取得方法
...
<TopicGrid>
  <AutoTopicCard title="ユーザー権限" href="/nipoplus/reference/userRank" />
  <AutoTopicCard title="アカウント管理" href="/nipoplus/reference/account" />
</TopicGrid>
...
- ユーザアカウントが削除されたとき
- ユーザアカウントで大きな変更が検知されたとき（パスワードやメールアドレスの変更など）

---

## APIを使い日報のデータを取得する

APIを利用する際は、ある程度の技術的な知識が必要です。

- CURL
- JSON

このあたりの基本が理解できれば問題ありません。本ガイドはCURLを使用しています。コマンドが苦手な方は[Postman](https://www.postman.com)などのGUIツールをご利用ください。

### APIへリクエストを投げる基本の形を日報取得の例から見る [#id=sample]
...
ここでは日報の取得に関するエンドポイントのみ紹介します。以下のいずれかの値を指定します。必須です。
...
#### エンドポイントとパラメータを指定した最小限のCurl [#id=smallest]
...
#### パラメータを色々指定した実用性のあるCurl [#id=long_sample]
...
<dl class="basic">
  ...
  <dt>tags</dt>
  <dd>
    タグによるフィルタを使う場合に指定します。タグ名ではなくタグのIDで指定します。タグのIDは<a href="/nipoplus/editor/tagManagement">タグ管理</a>から確認できます
  </dd>
  <dt>states</dt>
  <dd>
    <a href="/nipoplus/reference/reportstate">日報の状態</a>によるフィルタを使う場合に指定します。この例では新規、進行、棄却の日報のみを取得します（承認や修正の日報は除外）
  </dd>
  <dt>templates</dt>
...
### その他のエンドポイント [#id=other]

日報の取得以外に用意されているエンドポイントとパラメータについてまとめています。

#### テンプレートの取得エンドポイント [#id=template_Endpoint]
...
#### タイムカード取得エンドポイント [#id=timecard_endpoint]
...
このAPIポイントは[編集者権限](/nipoplus/reference/userRank/#editor)以上が必要です。
...
<dl class="basic">
  <dt>トークンの更新に失敗</dt>
  <dd>
    Bearerトークンが失効しています.<a href="/nipoplus/reference/api">APIキーの管理</a>から再度取得してください
  </dd>
  <dt>グループIDが不正です</dt>
...
## APIで取得されるデータの構造 [#id=data_structure]
...
| state      | String           | 承認・棄却・修正・新規・進行 いずれかの文字                                                                                                                         |
...
### タイムカードのJSON構造 [#id=timecard_JSON]
...
| redacted      | Boolean         | 編集済みフラグ。ユーザが編集した場合 true                                              |
...
#### calcData の中身 [#id=document_details]
...
- `memo` や `redacted` はユーザ操作により変更される要素です

#### タイムカードのAPIレスポンス例 [#id=timecard_result]
...
### テンプレートのJSON構造 [#id=template_JSON]
