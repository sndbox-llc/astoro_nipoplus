...
予め[<strong>登録しておいた商品</strong>](/nipoplus/editor/inventory/#add)（部品、資材、製品、備品など）を対象に、日報内で入出庫の入力を行うことで、**数量の増減管理**ができます。また、「<strong>状態</strong>」（例：仕掛中、完成、点検中）の管理も可能です。
...
商品入力フォームを使うには、予め管理対象となる商品データを[<strong>商品管理画面で登録しておく</strong>](/nipoplus/editor/inventory/)必要があります。詳細は「[商品在庫・状態管理](/nipoplus/editor/inventory/)」のページをご確認ください。
[<strong>実際の入力イメージはこちら</strong>](/nipoplus/reference/inputmethod/#item)からご確認ください。
...
現在の在庫総数は日報上には表示されませんが、「詳細を見る」ボタンをクリックすることで、[<strong>商品在庫・状態管理画面</strong>](/nipoplus/editor/inventory/#stackLog)にてリアルタイムの数量や状態、そしてその推移履歴を確認することが可能です。
