...
![テンプレートから「業務報告」の項目を誤って削除して保存してしまったとします。この操作が過去の日報に影響します](./img/preRemoveForm_tablet.webp)

削除後、同じタイトル（例：「業務報告」）で新しい入力フォームを作り直してテンプレートを保存しても、過去の日報では「業務報告」の入力データは表示されません。

![同じ名前のフォームを追加しても、過去の日報のデータは復旧しません。これは「キー」が異なるためです](./img/invalidKey_tablet.webp)
...
![過去の日報の「生データ」を表示します。ここには、日報に記録された全ての項目と、その「キー（識別番号）」が保存されています](./img/rawData_tablet.webp)
...
![手動で入力フォームの「キー（識別番号）」を変更できます。作業は慎重に、控えておいたキーを正確に入力してください](./img/rawData_tablet.webp)
