...
title: キーについて - 削除した入力項目の復旧
description: 誤ってテンプレートの一部を削除してしまい、過去の日報からデータが消えた場合でも復旧することが可能です。本ガイドでは復旧手順について触れています。
sidebar:
  label: 🔑キーについて
  badge:
    text: 応用
...
作業は細心の注意を払い、手順をよく理解してから行ってください。
:::

## 入力項目を誤って削除してしまった場合の復旧方法 {#about}

NipoPlusのテンプレートは[自由にカスタマイズ](/tips/templatemake)できるのが大きな特徴ですが、テンプレート内の入力フォームを削除する際は注意が必要です。
...
![テンプレートから「業務報告」の項目を誤って削除すると過去の日報に影響する](./img/preRemoveForm_tablet.webp)

削除後、同じタイトル（例：「業務報告」）で新しい入力フォームを作り直してテンプレートを保存しても、過去の日報では「業務報告」の入力データは表示されません。

![同じ名前のフォームを追加しても、過去の日報のデータは復旧しないのは「キー」が異なるため](./img/invalidKey_tablet.webp)
...
1.  データを確認したい過去の日報を表示する
2.  日報表示画面から「詳細表示」を開き、「生データを表示」ボタンをクリック

![過去の日報の「生データ」を表示し内容を確認する](./img/rawData_tablet.webp)
...
:::caution[注意：複雑なデータ構造の場合]
実際の生データはもっと数が多い場合や、[反復入力フォーム](/nipoplus/template/array)などが含まれている場合、データが入れ子構造となるためより複雑に見える可能性があります。その場合でも、入力した内容を手がかりに、その近くにある3〜5文字の英数字がキーです。一つ一つ慎重に照らし合わせて目的のデータを見つける必要があります。
:::
...
1.  テンプレート編集画面を開く
2.  修正したい入力フォーム（例：作り直した「業務報告」フォーム）の右下にあるスパナアイコンをクリックして設定画面を開く
3.  設定画面に表示される「キー」の欄に、控えておいた削除前の正確なキー（例：「DQQ」）を慎重に入力
4.  変更後、必ず「保存」ボタンをクリックしてテンプレートを保存する

![手動で入力フォームのキーを変更可能。作業は慎重に、控えておいたキーを正確に入力する](./img/rawData_tablet.webp)
