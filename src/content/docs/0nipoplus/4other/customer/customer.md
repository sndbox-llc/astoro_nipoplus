...
登録されている顧客一覧を表示するには、左メニュー「顧客一覧」をクリックします。
...
顧客一覧から目的の顧客を見つけたり、条件にマッチする顧客を見つける方法について解説します。
...
一番カンタンな検索から。とにかくキーワードを入れて、関連しそうな顧客をリストアップします。
...
検索条件の保存自体は無料PLANでもできますが、使用することは出来ません。
